...
| Area | Implemented |
| --- | --- |
| Control plane API | Dev auth, OIDC bearer validation, squads, agents, boards, tasks, registry, permissions, access grants, audit/metering reads, gateway metering callback, lease-backed/fenced agent task APIs, retry/dead-letter-aware message APIs, agent work-wait endpoint, context APIs, and Kubernetes outbox intents. |
...
| Agent runtime | Bootstrap/readiness, mounted Secret loading, task loop with control-plane work waiting plus fallback polling, task execution fence propagation, inbox draining, LiteLLM handler, importlib plugin loading, per-task context fetch, permission-filtered tool exposure, and bounded memory persistence. |
