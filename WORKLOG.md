...
## 2026-09-17 (Christian/Claude) - squad LLM and dialog review follow-ups

- objective: Address the review comments on the squad LLM and create-dialog changes (rossbrigoli/skquad#2).
- files changed: `web/src/app/page.tsx`, `web/src/app/globals.css`, `web/src/components/Modal.tsx`, `web/src/components/SquadsSection.tsx`, `web/src/components/shared.tsx`, `web/src/components/shared.test.ts`, `docs/web-app-ux.md`, `docs/implementation-status.md`, `docs/images/review-followups/`, `WORKLOG.md`.
- command/test run: `npm run lint`, `npx tsc --noEmit`, `npm run test:coverage` and `NEXT_TELEMETRY_DISABLED=1 npm run build` in `web/`; a 38-check headless-Chrome run against a mock control plane that injects failures into individual Apply and delete requests.
- result: Apply squad LLM now grants the squad provider before switching the agent's defaults and removes other provider grants last, so a failed step never leaves an agent pointing at a provider it has no grant for; a leftover grant keeps Apply offered so a rerun finishes the job. The rotate note is based on the grants read from the server. The squad LLM picker shows an unregistered provider instead of silently displaying the first option. When merged onto upstream `ca5eca1`, the upstream decision to allow unsetting the squad LLM was kept (the fork had briefly blocked it), and the three-step Apply replaced the upstream single-swap Apply while keeping its fresh-grants rotate notice. Squad, agent and task deletes confirm in the shared dialog with a red confirm button and keep errors inside it. The two architectural follow-ups (a first-class squad LLM field, and grant changes needing an identity rotation) are recorded in `docs/implementation-status.md`.
